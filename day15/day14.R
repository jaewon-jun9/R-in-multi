...
lon <- 127.0147559 #서울의 경도
lat <- 37.5431424  #서울의 위도
cen <- c(lon,lat)
...
ggmap(map)
map <- get_map(location=cen, maptype="watercolor",zoom=8, marker=mk)
ggmap(map)
...
save(station_lonlat, file="station_lonlat.rda") #여러번 쓸 것 같아서 저장한것
#load("station_lonlat.rda") 로 부르면 됨
