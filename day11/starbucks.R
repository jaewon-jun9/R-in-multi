...
addr <- NULL
for (i in 1:503){
  addrcss <- paste0("#mCSB_3_container > ul > li:nth-child(",i,") p[class^=result_details]")
...
addrr<- NULL
for(i in 1:503){
  addrr <- addrphones[[i]][1]
  addrss <- c(addrss,addrr)
}
addrss
...
starbucks <- list(shopname=shopnames,lat=lat,lng=lng,addr=addrss,telephone=phones)
