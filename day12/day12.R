...
test2-1 <- data.frame(id = c(1, 2, 5, 4, 3),  final = c(70, 83, 80, 95, 65))
# id 기준으로 합치기
total <- left_join(test1, test2, by = "id")  # id 기준으로 합쳐 total에 할당
#test2-1처럼 순서가 달라도 알아서 찾아서 합쳐줌
...
#상자그림 통계치 출력
boxplot(mpg$hwy)$stats  # 상자그림 통계치 출력 stats(통계)
...
as.Date("2018/01/02 08:00:00") - as.Date("2018/01/01 05:00:00")
...
unclass(ct) 
unclass(lt) 
lt$mon+1 #mon에 +1해줘야함 만든사람이 0부터 저장해서...
lt$hour
lt$year+1900
