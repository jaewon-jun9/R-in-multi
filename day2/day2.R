...
a1[1,3,4] #4층의 1행3열 데이터
a1[,,3] #3층의 모든 데이터
a1[,2,] #각 층에 2열 데이터 묶어서 행렬(기존구조 유지하고 싶으면 drop활용)
a1[1,,] #각 층마다 1행의 데이터 묶어서 행렬
...
btype <- factor(c("O","AB","B","O","A"),levels=c("A","B","O"))
btype
summary(btype)
levels(btype)
...
fruit$qty
fruit[[3]]
fruit[3]  # 데이터프레임 형식 유지 = frut[,3,drop=F]
...
names(df_midterm2)
df_midterm2
df_midterm2 <- data.frame(영어=c(90, 80, 60, 70), 수학=c(50, 60, 100, 20), 클래스=c(1,1,2,2))
df_midterm2
df_midterm2$영어
...
score$grade<-ifelse(score$sum >= 230,"A", ifelse(score$sum >= 215,"B", ifelse(score$sum >=200,"C","D")))
score
